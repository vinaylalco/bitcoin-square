  ...
  return (
    <div className="relative flex min-h-screen w-full overflow-hidden bg-gradient-to-br from-amber-50 via-white to-rose-50 dark:from-neutral-950 dark:via-neutral-950 dark:to-neutral-900">
      ...
      <div className="relative z-0 flex min-h-screen w-full">
        ...
        <div className="flex min-h-screen flex-1 flex-col backdrop-blur-sm">
          <header className="flex items-center justify-center border-b border-white/40 bg-white/30 px-4 py-4 backdrop-blur dark:border-neutral-800 dark:bg-neutral-900/60">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/80 p-1 shadow-sm backdrop-blur dark:bg-neutral-800/80">
              <button
                type="button"
            ...
            {isCasualView ? (
              <>
                <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-6 sm:px-8">
                  {loading && (
                    <p className="rounded-2xl bg-white/70 p-4 text-sm text-[var(--fg-muted)] shadow-sm dark:bg-neutral-900/60">
                ...
                <div className="sticky bottom-0 border-t border-white/50 bg-white/85 px-4 py-4 shadow-inner backdrop-blur dark:border-neutral-800 dark:bg-neutral-900/70 sm:px-8">
                  {!pubkey && (
                    <p className="mb-3 rounded-2xl border border-dashed border-white/60 bg-white/70 p-4 text-sm text-[var(--fg-muted)] shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60">
                      We&apos;re preparing your BitcoinSquare Nostr keys. Refresh the page if this message sticks around.
                    </p>
                  )}
                  {!hasRoomKey && (
                    <p className="mb-3 rounded-2xl border border-dashed border-white/60 bg-white/70 p-4 text-sm text-[var(--fg-muted)] shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60">
                      The shared casual room key isn&apos;t available right now. Please contact the BitcoinSquare team to restore access.
                    </p>
                  )}
