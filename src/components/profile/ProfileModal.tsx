...
const FALLBACK_ERROR_MESSAGE = "Profile unavailable. Try again.";
...
const useModalPortal = () => {
  ...
  useEffect(() => {
    ...
    let portal = document.getElementById("profile-modal-root");
    if (!portal) {
      portal = document.createElement("div");
      portal.id = "profile-modal-root";
...
const ProfileModalBody: React.FC<ProfileModalBodyProps> = ({
  ...
  const statusMessage = status === "error" ? error ?? FALLBACK_ERROR_MESSAGE : null;
...
const ProfileModal: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    return resolveProfileSummary(activeProfile);
  }, [activeProfile, resolveProfileSummary]);

  const following = useMemo(() => {
    if (!activeProfile) return false;
    return isFollowing(activeProfile);
  }, [activeProfile, isFollowing]);

  const ready = Boolean(activeProfile && summary && portalNode);
  if (!ready || !activeProfile || !summary || !portalNode) {
    return null;
  }
