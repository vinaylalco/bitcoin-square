...
interface ProgressCompatibleUser {
  points: number;
  lessonCompletions?: Record<string, string[] | undefined> | null;
  studyStreak: number;
  lastStudyDate: string | null;
}

export interface ProgressMergeResult {
  updatedCompletions: Record<string, string[]>;
  updatedPoints: number;
  updatedStudyStreak: number;
  updatedLastStudyDate: string | null;
  newCardCount: number;
}

export function computeMergedProgress(
  progress: LocalProgress,
  user: ProgressCompatibleUser,
): ProgressMergeResult | null {
  const basePoints = Number.isFinite(user.points) ? user.points : 0;
  const baseStreak =
  ...
  const updatedPoints = basePoints + newCardCount * 10;
  ...
  return {
    updatedCompletions,
    updatedPoints,
    updatedStudyStreak: nextStreak,
    updatedLastStudyDate: nextLastStudyDate,
    newCardCount,
  };
}
